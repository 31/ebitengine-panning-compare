...
func (g *Game) initAudioIfNeeded() {
	...
	leftStream := NewSingleChannelStream(NewSineStream(sampleRate), true)
	var err error
	...
	rightStream := NewSingleChannelStream(NewSineStream(sampleRate), false)
...
func (s *SineStream) Read(p []byte) (int, error) {
	...
	for i := 0; i < n; i += 8 {
		...
		// Right channel (identical signal; SingleChannelStream will zero out the unwanted one)
