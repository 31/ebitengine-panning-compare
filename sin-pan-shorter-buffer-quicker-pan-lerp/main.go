...
func (g *Game) initAudioIfNeeded() {
	...
	// Wrap an infinite sine-wave stream with the StereoPanStream
	g.panstream = NewStereoPanStream(NewSineStream(sampleRate))
...
func (g *Game) Update() error {
	g.count++
	r := float64(g.count) * ((1.0 / 60.0) * 2 * math.Pi) * 0.6
...
func (s *StereoPanStream) Read(p []byte) (int, error) {
	...
	for i := 0; i < alignedN; i += 8 {
		...
		ls := float32(math.Min(panForFrame*-1+1, 1))
		rs := float32(math.Min(panForFrame+1, 1))
...
// SineStream is an infinite stereo float32 PCM stream that mixes a 220 Hz
// and a 440 Hz sine wave at equal amplitudes.
type SineStream struct {
	sampleRate int
	pos        int64 // current sample frame index
}

func NewSineStream(sampleRate int) *SineStream {
	return &SineStream{sampleRate: sampleRate}
}

func (s *SineStream) Read(p []byte) (int, error) {
	n := len(p) / 8 * 8
	for i := 0; i < n; i += 8 {
		t := float64(s.pos) / float64(s.sampleRate)
		sample := float32(0.3*math.Sin(2*math.Pi*220*t) + 0.3*math.Sin(2*math.Pi*440*t))
		bits := math.Float32bits(sample)
		// Left channel
		p[i], p[i+1], p[i+2], p[i+3] = byte(bits), byte(bits>>8), byte(bits>>16), byte(bits>>24)
		// Right channel (identical signal; panning is applied by StereoPanStream)
		p[i+4], p[i+5], p[i+6], p[i+7] = byte(bits), byte(bits>>8), byte(bits>>16), byte(bits>>24)
		s.pos++
	}
	return n, nil
}

func (s *SineStream) Seek(offset int64, whence int) (int64, error) {
	switch whence {
	case io.SeekStart:
		s.pos = offset / 8
	case io.SeekCurrent:
